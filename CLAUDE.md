...
- **Role CV xác định (025)**: `CVData.role_category` quyết định MỘT LẦN lúc tạo CV (employee: skills+position qua `match_cv_data(position=...)`; CV thật: full text qua LLM parser) — engine dùng `_cv_role()` ở mọi chỗ, KHÔNG tự infer lại từ text (text tổng hợp "Skills: react..." từng kích hoạt title-regex làm role nhảy fullstack↔frontend giữa upload/rematch). Upload employee = rematch path (1 LLM call duy nhất lúc parse). Lưu ý: điểm hiển thị bị remap theo rổ riêng từng CV (A3) — KHÔNG so sánh số tuyệt đối giữa 2 employee.
