...
- **Match weights (feature 019)**: the hybrid score `α·GNN + β·skill + γ·seniority` weights are **tuned** by `python manage.py tune_hybrid_weights` (grid-search AUC on the labeled match/no_match pairs → ablation table) and loaded by the engine from checkpoint `metadata.json` `hybrid_weights` (single source of truth; `settings.py` values are training-only). The four per-dimension scores (skill/experience/seniority/domain fit) are **transparent formulas** in `engine._dimension_scores` (no learned aux head) — each reproducible by hand. Re-match (`rematch_employees`/`morning_refresh`) for existing rows to adopt new weights/dims; restart the server to load tuned weights.
