<!-- SPECKIT START -->
Active feature plan: [specs/029-i18n-frontend/plan.md](specs/029-i18n-frontend/plan.md)

Related artifacts:
- [spec.md](specs/029-i18n-frontend/spec.md)
- [research.md](specs/029-i18n-frontend/research.md)
- [data-model.md](specs/029-i18n-frontend/data-model.md)
- [contracts/i18n-contract.md](specs/029-i18n-frontend/contracts/i18n-contract.md)
- [quickstart.md](specs/029-i18n-frontend/quickstart.md)
- Master plan: [docs/codebase-knowledge/10-master-plan.md](docs/codebase-knowledge/10-master-plan.md)

Previous active plan: [specs/027-job-retrieval-scaling/plan.md](specs/027-job-retrieval-scaling/plan.md)
...
- **CV versions (027)**: mỗi `Employee` có nhiều `EmployeeCV` (PDF + parse RIÊNG), đúng 1 bản `is_active` (partial unique constraint). Engine KHÔNG đổi: parse của version active được **mirror lên các field của Employee** (`skills/position/seniority/experience_years/cv_file`) — nguồn `match_cv_data` đọc. Upload version mới = parse nền (thread pool, `_do_parse_cv_version`) nhưng KHÔNG tự active (trừ bản đầu); HR bấm "Dùng" → `activate_cv` copy lên Employee + rematch. API: `/api/admin/employees/{id}/cvs/` (GET/POST), `.../cvs/{cv_id}/activate/` (POST), `.../cvs/{cv_id}/` (DELETE, chặn xoá bản active). UI: card "CV versions" ở trang info nhân viên. Creation/rescore cũ vẫn giữ 1 active version đồng bộ qua `_sync_active_version_from_employee`.
