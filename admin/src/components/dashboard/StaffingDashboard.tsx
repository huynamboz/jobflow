...
function StatCard({
  ...
  return (
    <Card padding={16} className="flex flex-row items-center gap-3">
      <div className={`grid size-10 shrink-0 place-items-center rounded-xl ${TONE_RING[tone]}`}>{icon}</div>
      <div className="min-w-0">
        <div className={`text-2xl font-bold leading-none ${TONE_TEXT[tone]}`}>{value}</div>
...
function SectionTitle({
  ...
  return (
    <div className="mb-2 flex items-center gap-2">
      ...
      {count != null && count > 0 && (
        <span className="rounded-full bg-default-100 px-2 py-0.5 text-xs font-medium text-default-600">{count}</span>
      )}
    </div>
  );
}
            ...
            {funnelRows.map((f) => (
              <div key={f.key}>
                ...
                <div className="h-2 w-full overflow-hidden rounded-full bg-default-100">
                  <div className={`h-2 rounded-full ${f.color} transition-all`} style={{ width: `${(funnel[f.key] / funnelMax) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
